...
import type { Options } from "@typebot.io/docusaurus-preset-openapi";
...
export default function preset(
  ...
  themes.push(makePluginConfig("@typebot.io/docusaurus-theme-openapi"));

  if (api !== false) {
    plugins.push(
      makePluginConfig("@typebot.io/docusaurus-plugin-openapi", api)
    );
  }

  if (proxy !== false) {
    plugins.push(
      makePluginConfig("@typebot.io/docusaurus-plugin-proxy", proxy)
    );
  }
